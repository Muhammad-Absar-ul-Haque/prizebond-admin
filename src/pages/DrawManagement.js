...
export default function DrawManagement() {
  ...
  const fetchDraws = useCallback(async () => {
    setLoading(true);
    try {
      const response = await drawsService.listAll({
        page,
        limit,
        denomination: activeDenom,
      });
      setDraws(response.data || []);
      setMeta(response.meta || { total: response.length || 0, totalPages: 1 });
    } catch (err) {
  ...
  const stats = {
    total: meta.total,
    totalWinners: draws.reduce(
      (acc, d) => acc + (d._count?.winningNumbers || 0),
      0,
    ),
    ...
    <div style={s.page}>
      ...
      <div style={s.content}>
        ...
        <div style={s.toolbar}>
          <div style={s.filterGroup}>
            ...
            <div style={s.filterTabs}>
              <button
                style={{
                  ...s.filterTab,
                  ...(activeDenom === "ALL" ? s.filterTabActive : {}),
                }}
                onClick={() => {
                  setActiveDenom("ALL");
                  setPage(1);
              ...
              {DENOMINATIONS.map((d) => (
                <button
                  ...
                  style={{
                    ...s.filterTab,
                    ...(activeDenom === d.toString() ? s.filterTabActive : {}),
                  }}
                  onClick={() => {
                    setActiveDenom(d.toString());
                    setPage(1);
                  }}
                >
                  {d}
          ...
          <div style={s.actions}>
            <button style={s.refreshBtn} onClick={fetchDraws}>
              ↻
            </button>
            <button style={s.bulkBtn} onClick={() => setShowBulkImport(true)}>
              Bulk Import
            </button>
                  ...
                  <td colSpan={6} style={s.emptyCell}>
                    <div style={s.empty}>
                      ...
                      <div style={s.emptySub}>
                        Try creating a new draw or changing filters.
                      </div>
                    </div>
                  </td>
                </tr>
                  ...
                  >
                    <td style={s.td}>
                      ...
                      >
                        ...
                        {(draw.resultFileUrl || draw.hasResult) && (
                    ...
                    <td style={s.td}>
                      <div style={s.winnerCount}>
                        ...
                        {draw.totalWinners || draw._count?.winningNumbers || 0}
                      </div>
                    </td>
        ...
        {meta.totalPages > 1 && (
          <div style={s.pagination}>
            <div style={s.pageInfo}>
              Showing <b>{filtered.length}</b> of <b>{meta.total}</b> draws
            </div>
...
const s = {
  ...
  pageInfo: { fontSize: 12, color: "#6a8fad" },
  pageBtns: { display: "flex", gap: 6, alignItems: "center" },
  pageBtn: {
    padding: "6px 12px",
    borderRadius: 6,
    background: "#07090f",
  ...
  pageDots: { color: "#304d68", fontSize: 12 },
};
